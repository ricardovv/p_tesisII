...
void panelTopics() {
  ...
  int menuY = gridUnit*4;//position from top
  //IMPORTANT
  rectMode(CORNER);
...
//  articlesButtonsCategories(menuTopicX, menuTitleY+menuTitleH+menuItemH*i, "button n"+i);
  articlesButtonsCategories(menuTopicX, menuTitleY+menuTitleH+menuItemH*i, menuItemW, menuItemH, i, "Category n"+i);
...
void chaptersTopDisplay(){
  for (int i=0; i<chaptersNumber.length; i++) {
    ...
    if (  (mouseX>boxChapterX && mouseX<boxChapterX+articleBoxW-sep*5)  &&  (mouseY>boxChapterY && mouseY<boxChapterY+boxChapterH)  ) {
      chapterDetailsOverVisualization( mouseX, visualPosY, chaptersTitle[i], "Brief description about the current chapter. Brief description about the current chapter. " );
    }
  }//close for
...
void chapterDetailsOverVisualization( float _newPosBoxX, float _newPosBoxY, String _chapterTitle, String _chapterDescription ) {  
  ...
  pushMatrix();
    ...
    fill(50);
    textSize(12);
    textAlign(CENTER);textLeading(10);
    text(_chapterTitle, boxPosX+5, -4, boxW-10, boxH-15 );//TEXT TITLE
    textSize(10); textLeading(12);
    text(_chapterDescription, boxPosX+10, 24, boxW-20, boxH );//Text description
  popMatrix();
...
void articleDetailsOverVisualization( float _newPosBoxX, float _newPosBoxY, String _articleNumberText, int _articleWordsNum, String _articleTopics ) {  
  ...
  pushMatrix();
    ...
    text("click to view text", boxPosX+8, 105, boxW-15, boxH-15 );
  popMatrix();
...
void visualizationLabels(float _x, float _y){
  ...
  textAlign(LEFT, TOP);
  //TITLES
  rect(posX, posY+sepY*0, bW, bH);
    text("Label 1", posX+sepX, posY+sepY*0);
  rect(posX, posY+sepY*1, bW, bH);
    text("Label 2", posX+sepX, posY+sepY*1);
  rect(posX, posY+sepY*2, bW, bH);
    text("Label 3", posX+sepX, posY+sepY*2);
  rect(posX, posY+sepY*3, bW, bH);
